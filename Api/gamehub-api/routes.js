...
// Função auxiliar para centralizar o tratamento de erros em todas as rotas
const handleRequest = async (res, callback) => {
  try {
    ...
    if (!res.headersSent) {
      // Trata erros de constraint única (ex: CPF/email duplicado)
      if (err.code === "23505") {
        return res
          .status(409)
          .json({
            message:
              "Erro: Dados duplicados. CPF ou E-mail já podem estar cadastrados.",
            code: err.code,
          });
      }
      res
...
router.post("/clients", authMiddleware, async (req, res) => {
  await handleRequest(res, async () => {
    ...
    if (!name || !cpf || !agreed_to_terms) {
      return res
        .status(400)
        .json({ message: "Nome, CPF e aceite dos termos são obrigatórios." });
    }

    const sql = `
      INSERT INTO clients (
        name, birth_date, cpf, phone, email, address, neighborhood, city,
        client_type, hours_balance, subscription_status, subscription_date, next_billing_date,
        guardian_name, guardian_cpf, guardian_phone, guardian_relationship,
        agreed_to_terms, agreed_to_marketing
...
router.put("/clients/:id", authMiddleware, async (req, res) => {
  await handleRequest(res, async () => {
    ...
    const sql = `
      UPDATE clients SET 
        name = $1, birth_date = $2, cpf = $3, phone = $4, email = $5, address = $6, 
        neighborhood = $7, city = $8, client_type = $9, subscription_status = $10, 
        subscription_date = $11, next_billing_date = $12, guardian_name = $13, 
        guardian_cpf = $14, guardian_phone = $15, guardian_relationship = $16, 
        agreed_to_terms = $17, agreed_to_marketing = $18
      WHERE id = $19
    `;
    const params = [
      name,
...
// --- ROTAS DE AÇÕES DE CLIENTES (upgrade, renovação, etc.) ---

router.patch(
  "/clients/:id/upgrade-to-club",
  authMiddleware,
  async (req, res) => {
    await handleRequest(res, async () => {
      const { id } = req.params;
      ...
      if (!next_billing_date)
        return res
          .status(400)
          .json({ message: "A próxima data de cobrança é obrigatória." });
  ...
  async (req, res) => {
    await handleRequest(res, async () => {
      ...
      if (!new_next_billing_date)
        return res
          .status(400)
          .json({ message: "A nova data de cobrança é obrigatória." });
...
router.post("/sessions/check-in", authMiddleware, async (req, res) => {
  await handleRequest(res, async () => {
    ...
    const clientResult = await db.query(
      "SELECT hours_balance FROM clients WHERE id = $1",
      [client_id]
    );
    const client = clientResult.rows[0];
    if (!client)
      return res.status(404).json({ message: "Cliente não encontrado." });
    if (client.hours_balance <= 0)
      return res.status(400).json({ message: "Cliente sem saldo de horas." });

    const sessionResult = await db.query(
      "SELECT id FROM sessions WHERE client_id = $1 AND exit_time IS NULL",
      [client_id]
    );
    ...
    if (openSession)
      return res
        .status(409)
        .json({ message: "Este cliente já possui uma sessão de jogo ativa." });

    const entry_time = new Date();
    const insertResult = await db.query(
      "INSERT INTO sessions (client_id, entry_time) VALUES ($1, $2) RETURNING id",
      [client_id, entry_time]
    );
    res
      .status(201)
      .json({
...
router.post("/sessions/check-out", authMiddleware, async (req, res) => {
  await handleRequest(res, async () => {
    ...
    const { rows } = await db.query(
      `SELECT * FROM sessions WHERE client_id = $1 AND exit_time IS NULL`,
      [client_id]
    );
    ...
    if (!session)
      return res
        .status(404)
        .json({
          message: "Nenhuma sessão ativa encontrada para este cliente.",
        });

    const entryTime = new Date(session.entry_time);
    const exitTime = new Date();
    const durationMillis = exitTime - entryTime;
    const durationMinutes = Math.round(durationMillis / 60000);
    const durationHours = durationMillis / (1000 * 60 * 60);

    await db.query(
      `UPDATE sessions SET exit_time = $1, duration_minutes = $2 WHERE id = $3`,
      [exitTime, durationMinutes, session.id]
    );
    await db.query(
      `UPDATE clients SET hours_balance = hours_balance - $1 WHERE id = $2`,
      [durationHours, client_id]
    );

    res.json({
      message: `Check-out realizado. Duração: ${durationMinutes} minutos.`,
    });
  });
});
...
router.get("/sessions/client/:client_id", authMiddleware, async (req, res) => {
  await handleRequest(res, async () => {
    ...
    const { rows } = await db.query(
      "SELECT * FROM sessions WHERE client_id = $1 ORDER BY entry_time DESC",
      [client_id]
    );
    res.json(rows);
  });
...
router.get("/sessions/active", authMiddleware, async (req, res) => {
  await handleRequest(res, async () => {
    const sql = `
      SELECT s.id, s.entry_time, c.name, c.hours_balance, c.id AS client_id
      FROM sessions s
    ...
    const sessionsWithPrediction = rows.map((session) => {
      const entryTime = new Date(session.entry_time);
      const balanceInMilliseconds =
        parseFloat(session.hours_balance) * 60 * 60 * 1000;
      const predictedExitTimestamp =
        entryTime.getTime() + balanceInMilliseconds;
      return {
        ...session,
        predicted_exit_time: new Date(predictedExitTimestamp).toISOString(),
      };
    });
...
router.post(
  "/clients/:id/add-hours-transaction",
  authMiddleware,
  async (req, res) => {
    await handleRequest(res, async () => {
      ...
      if (!hours_to_add || !amount_paid) {
        return res
          .status(400)
          .json({ message: "Dados da transação inválidos." });
      }

      await db.query("BEGIN");
      await db.query(
        `UPDATE clients SET hours_balance = hours_balance + $1 WHERE id = $2`,
        [hours_to_add, client_id]
      );
      await db.query(
        `INSERT INTO transactions (client_id, transaction_type, hours_added, amount_paid, rate_used) VALUES ($1, 'ADD_HOURS', $2, $3, $4)`,
        [client_id, hours_to_add, amount_paid, rate_used]
      );
      ...
      res
        .status(201)
        .json({
          message: "Horas adicionadas e transação registrada com sucesso!",
        });
    });
  }
);
...
router.post("/clients/:id/buy-package", authMiddleware, async (req, res) => {
  await handleRequest(res, async () => {
    ...
    await db.query(
      `UPDATE clients SET hours_balance = hours_balance + $1 WHERE id = $2`,
      [hours_included, client_id]
    );
    await db.query(
      `INSERT INTO transactions (client_id, transaction_type, hours_added, amount_paid, notes) VALUES ($1, 'PACKAGE_PURCHASE', $2, $3, $4)`,
      [client_id, hours_included, price, `Compra do ${packageName}`]
    );
    await db.query("COMMIT");

    res
      .status(200)
      .json({ message: "Pacote comprado e horas adicionadas com sucesso!" });
  });
});
...
router.get("/stations", authMiddleware, async (req, res) => {
  await handleRequest(res, async () => {
    const { rows } = await db.query(
      "SELECT * FROM stations ORDER BY type, name"
    );
    res.json(rows);
  });
});
...
router.post("/stations", authMiddleware, async (req, res) => {
  await handleRequest(res, async () => {
    ...
    const params = [name, type, status || "AVAILABLE"];
    const { rows } = await db.query(sql, params);
    res
      .status(201)
      .json({ id: rows[0].id, message: "Estação criada com sucesso!" });
  });
});

// PUT - Atualizar uma estação
...
// GET - Listar pacotes ativos para venda
...
// GET - Listar TODOS os pacotes para o admin
...
router.post("/packages", authMiddleware, async (req, res) => {
  await handleRequest(res, async () => {
    ...
    res
      .status(201)
      .json({ id: rows[0].id, message: "Pacote criado com sucesso!" });
  });
});

// PUT - Atualizar um pacote
...
// GET - Busca agendamentos por data
...
router.post("/bookings", authMiddleware, async (req, res) => {
  await handleRequest(res, async () => {
    ...
    // Lógica para verificar se o horário já está ocupado
    const checkOverlapSql = `
      SELECT id FROM bookings
      WHERE station_id = $1 AND (start_time, end_time) OVERLAPS ($2, $3)
    `;
    const overlapResult = await db.query(checkOverlapSql, [
      station_id,
      start_time,
      end_time,
    ]);
    ...
    if (overlapResult.rows.length > 0) {
      return res
        ...
        .json({
          message:
            "Conflito de horário. Esta estação já está agendada para este período.",
        });
    }

    // Se não houver conflito, insere o agendamento
    ...
    res
      .status(201)
      .json({ id: rows[0].id, message: "Agendamento criado com sucesso!" });
  });
});
